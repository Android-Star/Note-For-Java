...
#### 122.线程会在如下几种情况释放对同步监视器的锁定：a、当前进程的同步方法、同步代码块执行结束；b、当前线程在同步方法、同步代码块中遇到break、return终止了执行；c、当前线程在同步代码块、同步方法出现未处理的error或Exception导致异常结束；d、当前线程#### 在执行同步代码块或者同步方法时，程序执行了同步监视器对象的wait()方法，则当前线程暂停，并释放同步监视器。
